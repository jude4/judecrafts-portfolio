<template>
    <ShuffleSection id="contact" :z-index="40" bg-class="bg-slate-100 dark:bg-black">
        <!-- Section Header -->
        <div class="text-center mb-12 w-full" v-animate-on-scroll="{
            initial: 'opacity-0 translate-y-8', final: 'opacity-100 translate-y-0'
        }">
            <h2 class="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
                Contact
            </h2>
            <h3 class="text-3xl md:text-4xl font-bold text-gray-800 dark:text-white mt-2">
        ...
        <div class="max-w-xl mx-auto">
            ...
            <!-- Error Message -->
            <div v-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md relative mb-6"
                role="alert">
            ...
            <!-- Form -->
            <form v-if="!success" @submit.prevent="handleSubmit" class="flex flex-col gap-6">
                <input v-model="formData.name" name="name"
                    ...
                    type="text" placeholder="Your Name" required v-animate-on-scroll="{
                ...
                <input v-model="formData.email" name="email"
                    class="p-3 w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-md focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"
                    type="email" placeholder="Your Email" required v-animate-on-scroll="{
                        initial: 'opacity-0 scale-95',
                        final: 'opacity-100 scale-100',
                        delay: 'delay-200'
                    }" />
                <textarea v-model="formData.message" name="message" rows="6"
                    class="p-3 w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-md focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"
                    placeholder="Your Message..." required v-animate-on-scroll="{
                        initial: 'opacity-0 scale-95',
                        final: 'opacity-100 scale-100',
                        delay: 'delay-300'
                    }"></textarea>
